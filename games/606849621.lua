...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/papawhomaomao-rgb/Crimsonware-V1/'..readfile('crimsonware/profiles/commit.txt')..'/'..select(1, path:gsub('crimsonware/', '')), true) end)
...
local vm = loadstring(downloadFile('crimsonware/libraries/vm.lua'), 'vm')()
